...
def splice_rnn_op(model_info, rnn_index):
    ...
    for index in range(layer_num):
        ...
        for bat in range(batch):
            ...
            sigmoid_output_name = 'lstm_' + str(index) + '_' + str(bat) + '.tmp_c'
            tanh_output_name = 'lstm_' + str(index) + '_' + str(bat) + '.tmp_h'
            ...
            sigmoid_op = {
                ...
                'outputs': {'Out': [sigmoid_output_name]},
                'type': 'rnn_sigmoid'
            }

            sigmoid_var = {
                'name': sigmoid_output_name,
                'persistable': False,
                'shape': [1, 1, weight_1_shape[1]]
            }

            ops.append(sigmoid_op)
            vars[sigmoid_output_name] = sigmoid_var
            ...
            tanh_op = {
                ...
                'outputs': {'Out': [tanh_output_name]},
                'type': 'rnn_tanh'
            }

            tanh_var = {
                'name': tanh_output_name,
                'persistable': False,
                'shape': [1, 1, weight_1_shape[1]]
            }

            ops.append(tanh_op)
            vars[tanh_output_name] = tanh_var

            last_hidden = tanh_output_name
            last_cell = sigmoid_output_name
