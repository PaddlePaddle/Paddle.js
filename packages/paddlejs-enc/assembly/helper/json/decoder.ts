...
//@ts-ignore: decorator
@lazy
const CHAR_P: i32 = 46; //".".charCodeAt(0);
//@ts-ignore: decorator
@lazy
...
export class JSONDecoder < JSONHandlerT extends JSONHandler > {
    ...
    private parseNumber(): bool {
        ...
        let digits = 0;
        while (CHAR_0 <= this.peekChar() && this.peekChar() <= CHAR_9 || this.peekChar() == CHAR_P) {
            let byte = this.readChar();
            if (byte == CHAR_P) {
                // 小数点
                isFloat = true;
        ...
        if (digits > 0) {
            if (isFloat) {
                this.handler.setFloat(
                    this.state.lastKey,
                    parseFloat((sign * integerPart).toString())
                        + parseFloat(number.toString()) / Math.pow(10, number.toString().length));
            }
            else {
